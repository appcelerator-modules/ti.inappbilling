...
public class InappbillingModule extends KrollModule
{
	...
	public void consume(HashMap hm)
	{
		...
		if (!(purchaseProxies instanceof Object[])) {
			throw new IllegalArgumentException("Invalid argument type `" + purchaseProxies.getClass().getName()
											   + "` passed to consume()");
		}
		...
		for (int i = 0; i < ((Object[]) purchaseProxies).length; i++) {
			...
			if (!(purchase instanceof PurchaseProxy)) {
				throw new IllegalArgumentException("Invalid argument type `" + purchase.getClass().getName()
												   + "` passed to consume()");
			}
	...
	IabHelper.OnConsumeMultiFinishedListener mConsumeMiltiFinishedListener =
			...
			public void onConsumeMultiFinished(List<Purchase> purchases, List<IabResult> results)
			{
				...
				if (hasListeners(CONSUME_COMPLETE)) {
					for (int i = 0; i < purchases.size(); i++) {
						logDebug("Consumption finished for Purchase: " + purchases.get(i)
								 + ", result: " + results.get(i));
						fireEvent(CONSUME_COMPLETE,
								  createEventObjectWithResult(results.get(i), null, purchases.get(i)));
					}
				}
			}
		};
