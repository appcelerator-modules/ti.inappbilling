...
public class IabHelper {
	 ...
	 *            Play Developer Console. Note that this is NOT your "developer
	 *            public key".
	 */
	...
	public void startSetup(final OnIabSetupFinishedListener listener) {
		...
		mServiceConn = new ServiceConnection() {
			...
			public void onServiceConnected(ComponentName name, IBinder service) {
				...
				try {
					logDebug("Checking for in-app billing "+API_VERSION+" support.");

					// check for in-app billing v* support
					...
					logDebug("In-app billing version "+API_VERSION+" supported for " + packageName);
	 ...
	 * method when you are done with this object. It will release any resources used
	 * by it such as service connections. Naturally, once the object is disposed of,
	...
	public interface OnIabPurchaseFinishedListener {
		 ...
		 * successful, then the sku parameter specifies which item was purchased. If the
		 * purchase failed, the sku and extraData parameters may or may not be null,
	...
	/**
	 * Initiate the UI flow for an in-app purchase. Call this method to initiate an
	 * in-app purchase, which will involve bringing up the Google Play screen. The
	 * calling activity will be paused while the user interacts with Google Play,
	 * and the result will be delivered via the activity's
	 * {@link android.app.Activity#onActivityResult} method, at which point you must
	...
	public void launchPurchaseFlow(Activity act, String sku, String itemType, int requestCode,
		...
		try {
			...
			// Appc: launching this pending intent using
			// TiActivitySupport.launchIntentSenderForResult rather than
			...
			TiActivityResultHandler wrapper = new TiActivityResultHandler() {
				public void onError(Activity activity, int requestCode, Exception e) {
					...
					if (listener != null)
						listener.onIabPurchaseFinished(errorResult, null);
				}
			...
			activitySupport.launchIntentSenderForResult(pendingIntent.getIntentSender(), requestCode, new Intent(),
					Integer.valueOf(0), Integer.valueOf(0), Integer.valueOf(0), null, wrapper);
		} catch (RemoteException e) {
	 ...
	 * this method from your Activity's
	 ...
	 *         handled; false if the result was not related to a purchase, in which
	...
	public Inventory queryInventory(boolean history, boolean querySkuDetails, List<String> moreSkus) throws IabException {
		return queryInventory(history,querySkuDetails, moreSkus, null);
	}
	 ...
	 */
	public Inventory queryInventory(boolean history,boolean querySkuDetails, List<String> moreItemSkus, List<String> moreSubsSkus)
			throws IabException {
		checkNotDisposed();
		checkSetupDone("queryInventory");
		try {
			Inventory inv = new Inventory();
			int r = queryPurchasesOrHistory(inv, ITEM_TYPE_INAPP,history);
			...
			if (mSubscriptionsSupported) {
				r = queryPurchasesOrHistory(inv, ITEM_TYPE_SUBS,history);
				if (r != BILLING_RESPONSE_RESULT_OK) {
	...
	public interface GetPurchaseHistoryFinishedListener {
		 ...
		 * 
		 */
		public void onGetPurchaseHistoryFinished(IabResult result, List<Purchase> purchases);
	}
	 ...
	 */
	// Appc: added this method to still have the same signature as the one below
	// that was changed
	public void queryInventoryAsync(final boolean querySkuDetails, final List<String> moreSkus,
			final QueryInventoryFinishedListener listener) {
		queryInventoryAsync(false,querySkuDetails, moreSkus, null, listener);
	}
	...
	// final QueryInventoryFinishedListener listener) {
	public void queryInventoryAsync(final boolean history,final boolean querySkuDetails, final List<String> moreItemSkus,
		...
		(new Thread(new Runnable() {
			public void run() {
				...
				Inventory inv = null;
				try {
					// Appc: changed the method that called here to allow passing subscription skus
					// as well
					// inv = queryInventory(querySkuDetails, moreSkus);
					inv = queryInventory(history,querySkuDetails, moreItemSkus, moreSubsSkus);
	...
	public void queryInventoryAsync(QueryInventoryFinishedListener listener) {
		queryInventoryAsync(false, false,null, null, listener);
	}
	 ...
	 * owned, and as a result of consumption, the user will no longer own it. This
	 * method may block or take long to return. Do not call from the UI thread. For
	 ...
	 * @param code
	 *            The response code
	 * @return A human-readable string explaining the result code. It also includes
	 *         the result code numerically.
	 */
	...
	int queryPurchasesOrHistory(Inventory inventury, String itemType, boolean history) throws JSONException, RemoteException {
		// Query purchases
		...
		do {
			...
			String packageName = mContext.getPackageName();
			ownedItems = (history) ? mService.getPurchases(API_VERSION, packageName, itemType, continueToken)
					: mService.getPurchaseHistory(API_VERSION, packageName, itemType, continueToken,null);
	...
	int querySkuDetails(String itemType, Inventory inv, List<String> moreSkus) throws RemoteException, JSONException {
		...
		Bundle skuDetails = mService.getSkuDetails(API_VERSION, mContext.getPackageName(), itemType, querySkus);
